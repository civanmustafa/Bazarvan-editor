...
test('Hostinger schema scripts and guide track the current production migrations', async () => {
  ...
  assert.match(applyScript, /EXPECTED_MIGRATIONS:-114/);
  assert.match(verifyScript, /EXPECTED_MIGRATIONS:-114/);
  ...
  assert.match(verifyScript, /EXTERNAL_ANALYSIS_DEPENDENCY_INVARIANT/);
  assert.match(guide, /20260905000000_manual_content_writing_priority\.sql/);
  assert.match(verifyScript, /MANUAL_WRITING_PRIORITY_FUNCTION/);
  assert.match(verifyScript, /MANUAL_WRITING_QUEUE_PRIVILEGES/);
  ...
  assert.match(guide, /20260913120916_content_writing_resume_provider_routing\.sql/);
  assert.match(verifyScript, /CONTENT_WRITING_RESUME_V2_FUNCTION/);
  assert.match(verifyScript, /CONTENT_WRITING_RESUME_V2_ACCESS/);
