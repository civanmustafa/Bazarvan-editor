...
test('content writing has one template registry and one context builder', async () => {
  ...
  assert.match(builder, /CONTENT_WRITING_MIN_COMPETITOR_WORDS = 0/);
  assert.match(builder, /CONTENT_WRITING_MIN_COMPETITOR_UNIQUE_TOKENS = 0/);
  assert.match(builder, /CONTENT_WRITING_MIN_DISTINCT_SOURCE_DOMAINS = 0/);
...
test('competitor coverage matrix and phrase intelligence are deterministic and controlled by the prompt registry', async () => {
  ...
  assert.match(promptRegistry, /PROMPT_REGISTRY_VERSION = 22/);
