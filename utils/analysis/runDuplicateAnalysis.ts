 ...
 * Target keywords, alternate forms, LSI terms, and the company name are
 * required editorial repetitions. Any repeated n-gram that contains one of
 * those terms (or a meaningful consecutive part of a multi-word term) is
 * excluded here and remains the responsibility of keyword-density analysis.
 */
      ...
      for (let n = 2; n <= 8; n++) {
        nGrams[n].forEach((value, key) => {
          if (value.locations.length > 1) {
            if (isProtectedTargetPhrase(key)) return;
