...
-- The two-meta-description workflow added the persisted step type to the table
-- constraint, but the RPC that prepares durable steps still had the older
-- allow-list. Automatic sessions therefore stopped immediately after the
-- outline when the worker eagerly prepared the remaining workflow steps.
