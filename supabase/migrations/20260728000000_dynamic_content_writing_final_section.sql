...
alter table public.content_writing_sessions
  add column if not exists dynamic_final_section_version smallint not null default 1
    check (dynamic_final_section_version >= 1);

comment on column public.content_writing_sessions.dynamic_final_section_version is
  'Database capability marker for goal-aware conclusion and call-to-action workflow stages.';
...
-- Preserve resumability for sessions created before the dynamic final-section
-- release. Only unfinished service/commercial sessions are reset at the old
-- conclusion step; completed historical results remain immutable.
update public.content_writing_steps as step
set step_key = 'call-to-action',
    step_type = 'call_to_action',
    ...
    metadata = coalesce(step.metadata, '{}'::jsonb) || jsonb_build_object(
      'workflowVersion', 7,
      'finalSectionKind', 'call_to_action'
    ),
...
where session.id = step.session_id
  and session.status in ('queued', 'running', 'retry_scheduled', 'failed')
  and step.step_key = 'conclusion'
  and lower(coalesce(session.context_snapshot #>> '{qualityInput,goalContext,pageType}', '')) in (
    'service',
    'category',
    'product',
    'landing'
