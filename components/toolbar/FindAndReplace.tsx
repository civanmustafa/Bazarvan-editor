...
const FindAndReplace: React.FC<FindAndReplaceProps> = ({ editor, t, clearAllHighlights, onClose }) => {
    ...
    return (
        <div className="flex items-center gap-2 p-1.5 bg-gray-200 dark:bg-[#2A2A2A] rounded-md">
            <input ref={findInputRef} type="text" placeholder={`${t.find}...`} value={findValue} onChange={e => { setFindValue(e.target.value); findAndHighlight(e.target.value); }} className="p-1 w-32 text-xs bg-white text-gray-800 placeholder:text-gray-400 dark:bg-[#1F1F1F] dark:text-gray-100 dark:placeholder:text-gray-500 rounded-md border-gray-300 dark:border-[#3C3C3C] focus:ring-0 focus:border-[#d4af37]" />
            ...
            <input type="text" placeholder={`${t.replaceWith}...`} value={replaceValue} onChange={e => setReplaceValue(e.target.value)} className="p-1 w-32 text-xs bg-white text-gray-800 placeholder:text-gray-400 dark:bg-[#1F1F1F] dark:text-gray-100 dark:placeholder:text-gray-500 rounded-md border-gray-300 dark:border-[#3C3C3C] focus:ring-0 focus:border-[#d4af37]" />
