        ...
        <div className="relative" ref={aiMenuRef}>
            ...
            {isAiMenuOpen && (
                <div className={`absolute mt-2 w-60 origin-top-left rounded-md bg-white dark:bg-[#2A2A2A] shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none py-1 z-30 ${uiLanguage === 'ar' ? 'left-0' : 'right-0'}`}>
                    ...
                    <AiMenuItem onClick={() => handleAiRequest(AI_PROMPTS.GENERATE_META, 'copy-meta')} disabled={isAnyGeminiLoading}><Tag size={14} /> <span>{t.aiMenu.generateMeta}</span></AiMenuItem>
                    <div className="my-1 h-px bg-gray-200 dark:bg-[#3C3C3C]"></div>

                    <AiMenuItem onClick={() => handleAiRequest(AI_PROMPTS.REPHRASE, 'replace-text')} disabled={!hasSelection || isAnyGeminiLoading}><PenLine size={14} /> <span>{t.aiMenu.rephrase}</span></AiMenuItem>
                    <AiMenuItem onClick={() => handleAiRequest(AI_PROMPTS.IMPROVE_WORDING, 'replace-text')} disabled={!hasSelection || isAnyGeminiLoading}><Wand2 size={14} /> <span>{t.aiMenu.improveWording}</span></AiMenuItem>
                    <AiMenuItem onClick={() => handleAiRequest(AI_PROMPTS.SIMPLIFY_TEXT, 'replace-text')} disabled={!hasSelection || isAnyGeminiLoading}><Zap size={14} /> <span>{t.aiMenu.simplify}</span></AiMenuItem>
                    
                    <div className="relative" onMouseEnter={() => setIsExpandMenuOpen(true)} onMouseLeave={() => setIsExpandMenuOpen(false)}>
                        <AiMenuItem onClick={() => handleAiRequest(AI_PROMPTS.EXPAND, 'replace-text')} disabled={!hasSelection || isAnyGeminiLoading}>
                            <Expand size={14} />
                        ...
                        {isExpandMenuOpen && (
                            <div className={`absolute top-0 ms-1 w-48 origin-top-left rounded-md bg-white dark:bg-[#2A2A2A] shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none py-1 ${uiLanguage === 'ar' ? 'left-full' : 'right-full'}`}>
                                <AiMenuItem onClick={() => handleAiRequest(AI_PROMPTS.EXPAND_50, 'replace-text')} disabled={!hasSelection || isAnyGeminiLoading}><span>{t.aiMenu.expand50}</span></AiMenuItem>
                                <AiMenuItem onClick={() => handleAiRequest(AI_PROMPTS.EXPAND_100, 'replace-text')} disabled={!hasSelection || isAnyGeminiLoading}><span>{t.aiMenu.expand100}</span></AiMenuItem>
                            </div>
                        )}
                    </div>
                    
                    <div className="relative" onMouseEnter={() => setIsSummarizeMenuOpen(true)} onMouseLeave={() => setIsSummarizeMenuOpen(false)}>
                        <AiMenuItem onClick={() => handleAiRequest(AI_PROMPTS.SUMMARIZE, 'replace-text')} disabled={!hasSelection || isAnyGeminiLoading}>
                            <BookText size={14} />
                        ...
                        {isSummarizeMenuOpen && (
                            <div className={`absolute top-0 ms-1 w-48 origin-top-left rounded-md bg-white dark:bg-[#2A2A2A] shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none py-1 ${uiLanguage === 'ar' ? 'left-full' : 'right-full'}`}>
                                <AiMenuItem onClick={() => handleAiRequest(AI_PROMPTS.SUMMARIZE_50, 'replace-text')} disabled={!hasSelection || isAnyGeminiLoading}><span>{t.aiMenu.summarize50}</span></AiMenuItem>
                                <AiMenuItem onClick={() => handleAiRequest(AI_PROMPTS.SUMMARIZE_100, 'replace-text')} disabled={!hasSelection || isAnyGeminiLoading}><span>{t.aiMenu.summarize100}</span></AiMenuItem>
                            </div>
                        )}
                    </div>
                    <AiMenuItem onClick={() => handleAiRequest(AI_PROMPTS.FIND_STATS, 'replace-text')} disabled={!hasSelection || isAnyGeminiLoading}><TestTube size={14} /> <span>{t.aiMenu.findStats}</span></AiMenuItem>

                    <div className="my-1 h-px bg-gray-200 dark:bg-[#3C3C3C]"></div>
                    <AiMenuItem onClick={() => handleAiRequest(AI_PROMPTS.TO_QA, 'replace-text')} disabled={!hasSelection || isAnyGeminiLoading}><HelpCircle size={14} /> <span>{t.aiMenu.toQA}</span></AiMenuItem>
                    <AiMenuItem onClick={() => handleAiRequest(AI_PROMPTS.TO_STEPS, 'replace-text')} disabled={!hasSelection || isAnyGeminiLoading}><ListChecks size={14} /> <span>{t.aiMenu.toSteps}</span></AiMenuItem>
                    <AiMenuItem onClick={() => handleAiRequest(AI_PROMPTS.TO_TABLE, 'replace-text')} disabled={!hasSelection || isAnyGeminiLoading}><Table size={14} /> <span>{t.aiMenu.toTable}</span></AiMenuItem>
                    <div className="my-1 h-px bg-gray-200 dark:bg-[#3C3C3C]"></div>
                    <div className="relative" onMouseEnter={() => setIsToneMenuOpen(true)} onMouseLeave={() => setIsToneMenuOpen(false)}>
                        ...
                        {isToneMenuOpen && (
                            <div className={`absolute top-0 ms-1 w-40 origin-top-left rounded-md bg-white dark:bg-[#2A2A2A] shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none py-1 ${uiLanguage === 'ar' ? 'left-full' : 'right-full'}`}>
                                {TONES.map(tone => (
                                    <AiMenuItem key={tone} onClick={() => handleAiRequest(AI_PROMPTS.CHANGE_TONE(tone), 'replace-text')} disabled={!hasSelection || isAnyGeminiLoading}>
                                        <span>{tone}</span>
                                    </AiMenuItem>
                                ))}
                            </div>
