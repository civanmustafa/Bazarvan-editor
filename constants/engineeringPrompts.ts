...
- لا تنشئ تقريرًا نصيًا أو ملخصًا تنفيذيًا أو مصفوفة مقارنة مختصرة أو حكمًا نهائيًا مستقلًا.
- اجعل analysisMarkdown سلسلة فارغة حرفيًا، وضع كل نتيجة مفيدة داخل بطاقة patch مستقلة قابلة للتطبيق في المحرر.
- رتّب البطاقات حسب الأثر. يجب أن تحتوي كل بطاقة على عنوان الإجراء، وموضعه الدقيق، والنص النهائي الجاهز، وسبب مختصر يذكر الفجوة والأدلة.
- حوّل فجوات الأسئلة والمقارنة أو الاختيار والاعتراضات والتعريفات والخطوات أو الأمثلة والكيانات والثقة والتحويل إلى نصوص نهائية، لا إلى ملاحظات أو نصائح.
- عند وجود ادعاءات أو معلومات متعارضة، لا تكتب «نقطة التعارض» و«التحليل والتحقق» كنص توضيحي. أنشئ بطاقة تصحيح أو استبدال بصياغة نهائية دقيقة وآمنة متى سمحت الأدلة.
- إذا كان التعارض يحتاج تحققًا خارجيًا ولا يمكن إنتاج نص نهائي آمن، استبعده من التعديل مع سبب محدد داخل itemDispositions، ولا تعرضه للمستخدم كفقرة عامة.
- اجعل فرص الجدول أو القائمة أو الخطوات بطاقة تحتوي البنية والمحتوى الجاهز، لا مجرد اقتراح باستخدامها.
- إذا كانت المشكلة في ترتيب H2 ولا يمكن تطبيق النقل بأمان، لا تنشئ شرحًا عامًا لها. عالجها فقط عندما تستطيع إنشاء بطاقة استبدال واضحة قابلة للتطبيق تتضمن الترتيب المقترح لعناوين H2.
- اجعل أضعف قسم بطاقة استبدال كاملة عند توفر تحسين حقيقي، ويمكن أن يتضمن reason حكمًا نهائيًا مختصرًا على أثر هذا التعديل بدل إنشاء تقرير منفصل.
...
- ضع النص الجاهز داخل patches فقط، ولا تضع title أو reason أو placementLabel أو contentMarkdown داخل analysisMarkdown لأنه يجب أن يبقى فارغًا.
- لا تنشئ patch لملاحظة تحتاج مراجعة يدوية أو تحققًا خارجيًا ولا يتوفر لها نص نهائي آمن.
- يجب أن تقابل كل مجموعة دمج retained أو merged بطاقة patch واحدة مستقلة على الأقل، أما النتائج التي لا تحتاج تعديلًا فتكون excluded مع سبب صريح.
- اربط كل بطاقة ناتجة عن الدمج بحقل clusterId وبقائمة sourceItemIds الكاملة، حتى يمكن إثبات أن كل فكرة قابلة للتطبيق لها بطاقة موضع ونسخ وتطبيق.
- لا تجمع فكرتين أو قسمَي H2 مستقلين في بطاقة واحدة؛ أنشئ cluster وبطاقة منفصلين لكل فكرة مستقلة.
...
export const sanitizeEngineeringPrompt = (id: EngineeringPromptId, value: string): string => {
  ...
  if (
    id === ENGINEERING_PROMPT_IDS.smartAnalysis.competitorContentComparison
    && value.includes('صيغة التقرير النهائي:')
    && value.includes('ملخص تنفيذي من 5–8 نقاط')
    && value.includes('الادعاءات أو المعلومات المتعارضة وما يحتاج تحققًا')
  ) {
    return DEFAULT_ENGINEERING_PROMPTS[id];
  }
