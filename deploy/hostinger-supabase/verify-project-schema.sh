...
readonly ARTICLE_COMPETITOR_SOURCE_POLICY="$(sql_scalar "select count(*) = 2 from information_schema.columns where table_schema = 'public' and table_name = 'article_competitors' and column_name in ('source_class','content_weight') and not exists(select 1 from public.article_competitors where source_class is distinct from 'commercial' or content_weight is distinct from 1.000)")"
...
[[ "${ARTICLE_COMPETITOR_SOURCE_POLICY}" == "t" ]] || fail "Competitor source class or writing weight is missing."
...
readonly MANUAL_COMPETITOR_REPLACEMENT_FUNCTION="$(sql_scalar "select to_regprocedure('public.enqueue_manual_competitor_extraction_job(uuid,uuid,text,text,jsonb,boolean)') is not null")"
[[ "${MANUAL_COMPETITOR_REPLACEMENT_FUNCTION}" == "t" ]] || fail "Explicit manual competitor replacement function is missing."
...
readonly MANUAL_COMPETITOR_REPLACEMENT_PRIVILEGES="$(sql_scalar "select has_function_privilege('anon', 'public.enqueue_manual_competitor_extraction_job(uuid,uuid,text,text,jsonb,boolean)', 'execute') or has_function_privilege('authenticated', 'public.enqueue_manual_competitor_extraction_job(uuid,uuid,text,text,jsonb,boolean)', 'execute')")"
[[ "${MANUAL_COMPETITOR_REPLACEMENT_PRIVILEGES}" == "f" ]] || fail "Manual competitor replacement must remain behind the write-access API."
