...
المرحلة العاشرة والأخيرة لا تضيف ترحيلًا سابعًا ولا أمرًا هندسيًا جديدًا؛ فهي بوابة اختبارات ونشر نهائية. إذا كانت ترحيلات مركز العملاء الستة مطبقة بالفعل، فلا تنفذ أي SQL إضافي بسبب هذه المرحلة. أصبح `npm run build` يشغّل تلقائيًا **13 حالة قبول** ويوقف النشر عند فشل أي منها: الصلاحيات، حصر الروابط بدومينات العميل، تنظيف التكرار، التحويلات وCanonical، استبعاد `noindex` و404، العربية والإنجليزية، غياب الوصف، العناوين العامة، دقة Anchor Text، عدم استخدام مقالات المحرر، عمل المحرك دون مفتاح API، حفظ التعديلات اليدوية عند إعادة الزحف، واستئناف مهام الزحف الفاشلة.
