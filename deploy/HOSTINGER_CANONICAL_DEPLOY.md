...
المرحلة التاسعة لا تضيف ترحيلًا سابعًا. إذا كنت قد نفذت الترحيلات الستة السابقة وترحيل سجل الأوامر الهندسية `20260724000000_admin_prompt_registry.sql`، فلا تنفذ SQL جديدًا لهذه المرحلة؛ اسحب آخر إصدار وابنِ التطبيق وأعد تحميل PM2 فقط. تضيف هذه المرحلة مراجعة اختيارية بالذكاء الاصطناعي لأفضل نتائج المحرك الخوارزمي، وتكون معطلة افتراضيًا ولا تعمل تلقائيًا. يُقرأ أمرها العربي من **الإعدادات ← الأوامر الهندسية ← الربط الداخلي**، ولا تستطيع المراجعة إضافة صفحة أو رابط أو Anchor Text خارج النتائج والبيانات الموجودة في مركز العميل.
...
إذا سبق أن نفّذت الترحيلات الخمسة الأولى، فلا تعِد تنفيذها؛ نفّذ الترحيل السادس الجديد فقط قبل تحديث الواجهة والخادم. لا يحتاج الترحيل السادس إلى مفتاح ذكاء اصطناعي أو Search Console أو عملية PM2 جديدة. بعد النشر، يعيد `/readyz` حالة HTTP 503 إذا كانت جداول مركز العملاء أو أعمدة الزحف أو جداول الربط الداخلي أو الفهرس الخوارزمي أو سجل اقتراحات المحرر أو سياسة الجودة المطلوبة غير متوفرة.
